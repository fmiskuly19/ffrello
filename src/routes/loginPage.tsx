...
const LoginPage = () => {
    ...
    useEffect(() => {
        if (ffrelloLoginStatus == ApiCallStatus.Loading) {
            //show loading spinner or something
        }
        else if (ffrelloLoginStatus == ApiCallStatus.Success) {
            enqueueSnackbar('Logged into FFrello!', { variant: 'success' });
            //set it back to idle
            dispatch(setFfrelloAuthenticationApiCallStatus(ApiCallStatus.Idle));
        }
                                ...
                                <Stack direction="column" spacing={1}>
                                    <GoogleLogin
                                        onSuccess={tokenResponse => {
                                            ...
                                            console.log(tokenResponse);
                                            dispatch(setIsAuthenticatedWithExternalProvider(true));
                                            dispatch(authenticateWithFFrelloApi(tokenResponse.credential as string))
                                        }}
                                        onError={() => {
                                            dispatch(setIsAuthenticatedWithExternalProvider(false));
                                            enqueueSnackbar('Google Login Failed', { variant: 'error' });
